...
class LocalCamNet(nn.Module):
    ...
    def forward(self, x, y=None, is_vis=False, vis_idx=None, gt_top=None):
        ...
        conv5 = self.conv5(conv4) # [batch_size, 2048, h, w]
        conv5_pool = self.pool(conv5).view(b, -1) # [batch_size, 2048]
        logits = self.classifier(conv5_pool) # [batch_size, num_classes]

        # ---
        pool_conv6 = self.pool_max(F.relu(self.conv6(conv4.detach()))).view(b, -1) # [batch_size, 10 * num_classes]
        pool_cat = torch.cat((10*l2_norm_v2(conv5_pool.detach()), 10*l2_norm_v2(pool_conv6.detach())), dim=1) # [batch_size, (10 * num_classes + 2048]

        logits_max = self.cls_part(pool_conv6) # [batch_size, num_classes]
        logits_cat = self.cls_cat(pool_cat) # [batch_size, num_classes]
        ...
        pr_gate = F.softmax(self.cls_gate(pool_gate), dim=1) # [batch_size, 3]

        logits_gate = torch.stack([logits_cat.detach(), logits_cat_1.detach(), logits_cat_2.detach()], dim=-1) # [batch_size, num_classes, 3]
        logits_gate = logits_gate * pr_gate.view(pr_gate.size(0), 1, pr_gate.size(1)) # [batch_size, 1, 3]
